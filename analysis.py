...
def main():
    ...
    while True:
        percent_black, funding = get_school_input()
        if percent_black is None or funding is None:
            continue

        report_count += 1
        analysis = analyze_school(percent_black, funding)
        print(analysis)

        save = input("Would you like to save this report? (yes/no): ").strip().lower()
        if save == "yes":
            save_report(analysis, report_count)

        again = input("Would you like to analyze another school? (yes/no): ").strip().lower()
        if again != "yes":
            break
